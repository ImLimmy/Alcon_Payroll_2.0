...
class User(AbstractUser):
    ...
    pag_ibig_contribution = models.ForeignKey(
        PagIbig, on_delete=models.CASCADE, null=True, blank=True)

    philhealth_number = models.IntegerField(
        unique=True, null=True, blank=False)
    philhealth_contribution = models.ForeignKey(
        PhilHealth, on_delete=models.CASCADE, null=True, blank=True)

    sss_number = models.IntegerField(unique=True, null=True, blank=False)
    sss_contribution = models.ForeignKey(
        SSS, on_delete=models.CASCADE, null=True, blank=True)
    ...
    def sss_contributions(self) -> float:
        sss_account = SSS.objects.filter(
            minimum_salary__lte=self.basic_pay, maximum_salary__gte=self.basic_pay).first()
